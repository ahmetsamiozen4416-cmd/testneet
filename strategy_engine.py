...
fetch_binance_data() artık structured dict döndürür (metin rapor değil).
"""
...
# ---------------------------------------------------------------------------
# SABITLER — data_collector v17 ile senkron
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SKOR MOTORU — tek fonksiyon, hem LONG hem SHORT
# ---------------------------------------------------------------------------
...
def score_coin(
    *,
    ...
    """
    Döner: (skor, reasons_list)
    reasons_list — her sinyal için insan-okunur açıklama.
    main.py bu skoru ASLA değiştirmez.
    """
    skor      = 0
    reasons   = []
    ...
    # ---- EMA200 ----
    # Veri: EMA200 altında LONG açılan coinler tutarsız sonuç verdi (SIREN, ZEC vs)
    # EMA200 altında trend yapısal olarak bozuk — LONG için skor cap değil, tam blok
    if is_long:
        if ema200_label == "ALTINDA❌":
            rsi_cap = 0   # skor ne olursa olsun 0'a inecek = blok
    ...
    if is_long:
        if change_24h < MIN_LONG_MOMENTUM_PCT:
            ...
            # Veri: 24s %40+ coinler geriye döndü (BOME %58, UAI %49, CHIP %41 hep kaybetti)
            rsi_cap = min(rsi_cap, 6)
    ...
    if rsi_15m is not None:
        if is_long:
            if rsi_15m > 85:
                rsi_cap = min(rsi_cap, 7)   # skor tavanı düş, blok değil ama kap
...
# ---------------------------------------------------------------------------
# ANA VERİ FONKSİYONU — structured dict döndürür (metin değil)
# ---------------------------------------------------------------------------

def fetch_binance_data(recently_closed_coins: set = None) -> dict:
    """
    Tüm piyasa taraması → structured dict.
      ...
      "candidates": [
        {
          ...
          "score": int,
          "reasons": list[str],
          "features": { rsi_15m, rsi_1h, rsi_4h, atr_ratio, vol_ratio,
                        ema200, stoch_k, macd_hist, price },
          "meta": { change_24h, volume, trend, ema200_label, ... }
        }
      ],
      "error": str | None
    }
    ...
    try:
        ...
        btc_change = float(btc_t.get('percentage', 0) or 0)
        eth_change = float(eth_t.get('percentage', 0) or 0) if eth_t else 0.0
        ...
        top_vol  = sorted(altcoin_pairs, key=lambda x: float(x.get('quoteVolume', 0) or 0),
                          reverse=True)[:TOP_BY_VOLUME_A]
        top_move = sorted(altcoin_pairs, key=lambda x: abs(float(x.get('percentage', 0) or 0)),
        ...
        for pair in candidates_raw:
            ...
            try:
                ...
                close      = feat['close']
                change_24h = float(pair.get('percentage', 0) or 0)

                # ---- 1h RSI (ayrı API çağrısı — sadece finalist adaylar için yapılacak) ----
                # Önce placeholder, 4h RSI ile beraber finalist listesinde çekiliyor
                rsi_1h = None   # finalist aşamasında doldurulacak
                ...
                # ---- Skor (1h RSI = None ile ilk hesap; finalist'te güncellenecek) ----
                ...
                base_entry = {
                    ...
                    "features": {
                        ...
                        "rsi_1h":   None,   # finalist'te doldurulacak
                        "rsi_4h":   None,   # finalist'te doldurulacak
        ...
        # ---- Finalist için 1h + 4h RSI çek, skoru YENİDEN HESAPLA ----
        # İlk skor rsi_1h=None ile hesaplandı — bu geçici bir ön-sıralamadır.
        # Gerçek 1h/4h RSI çekildikten sonra final skor burada güncellenir.
        for result_list in [long_results, short_results]:
            for r in result_list:
                try:
                    ...
                    # ---- Skoru gerçek RSI değerleriyle yeniden hesapla ----
                    ...
                    new_score, new_reasons = score_coin(
                        ...
                        macd_hist_prev=None,   # ilk hesapla tutarlı
                        price=feat_snap.get('price', 0),
                        ema20=feat_snap.get('ema200', feat_snap.get('price', 0)),  # ema20 burada yok, ema200 proxy
                        bb_upper=None, bb_lower=None,   # BB finalist'te yok, ilk skor karar verdi
        ...
        # Veri: skor 6-7 ile açılan tradesların neredeyse tamamı zarar (PORTAL skor6, UAI skor6, EUL skor6)
        # Finalist eşiğini 8'e çıkar — AI zaten 8+ istiyor ama liste kirleniyordu
