...
def execute_trade(
    ...
    try:
        ...
        try:
            md.exchange().fapiPrivatePostMarginType(
                {'symbol': market_info['id'], 'marginType': 'ISOLATED'})
        except: pass
        try:
        ...
        try:
            ...
            md.exchange().create_order(
                symbol=market_symbol, type='TAKE_PROFIT_MARKET', side=tp_s,
                amount=amount_coin, params={'stopPrice': tp_price, 'reduceOnly': True})
        except Exception as e:
            if send_telegram_fn:
                send_telegram_fn(f"⚠️ TP Hatası ({market_symbol}): {e}")
        ...
        try:
            ...
            md.exchange().create_order(
                symbol=market_symbol, type='STOP_MARKET', side=sl_s,
                amount=amount_coin, params={'stopPrice': sl_price, 'reduceOnly': True})
            sl_opened = True
        except Exception as e:
            if '-2021' in str(e):
                cs = 'SELL' if side == 'BUY' else 'BUY'
                try:
                    md.exchange().create_market_order(
                        symbol=market_symbol, side=cs, amount=amount_coin,
                        params={'reduceOnly': True})
                    record_trade_close(market_symbol, False)
                except Exception as ce:
                    if send_telegram_fn:
                        send_telegram_fn(f"🛑 Acil kapama ({market_symbol}): {ce}")
                return f"⚠️ SL hatası — kapatıldı: {market_symbol}"
            else:
                if send_telegram_fn:
                    send_telegram_fn(f"⚠️ SL Hatası ({market_symbol}): {e}")
