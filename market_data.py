...
⚙️  TESTNET MODU: Bybit Testnet
    API Key → https://testnet.bybit.com → API Management
    .env dosyasına şunları yaz:
        BINANCE_API_KEY=bybit_testnet_api_key
        BINANCE_SECRET_KEY=bybit_testnet_secret_key
    Gerçeğe geçmek için TESTNET = False yap.
...
# ---- Testnet bayrağı: False → gerçek Bybit ----
TESTNET = True
...
def get_exchange() -> ccxt.Exchange:
    config = {
        ...
        'options': {
            'defaultType': 'linear',   # Bybit linear perpetual (USDT-margined)
        },
    }
    if TESTNET:
        config['options']['testnet'] = True   # ccxt built-in Bybit testnet desteği
